/**
 * SERAVIEL LABS Toast System v2.2
 * 0 API, 0 Emojis, SVG Icons, Queue, Progress, Promises, Accessible
 * Features: auto-dismiss, pause on hover, actions with onClick/autoClose, promise wrapper
 */
(function(global) {
  'use strict';
  ...
  var ICONS = {
    ...
    loading: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 12a9 9 0 11-6.22-8.55"/></svg>',
  ...
  function createToastEl(opts) {
    ...
    for (var i = 0; i < actionBtns.length; i++) {
      (function(btn, actionDef) {
        btn.addEventListener('click', function(e) {
          e.stopPropagation();
          
          // 1. Global onAction handler (if provided)
          if (el._onAction && typeof el._onAction === 'function') {
            el._onAction(actionDef.id || actionDef.label, opts.id);
          }
          
          // 2. Individual onClick handler (if provided on action)
          if (actionDef.onClick && typeof actionDef.onClick === 'function') {
            actionDef.onClick(opts.id);
          }
          
          // 3. Auto-close logic (default true, unless autoClose: false)
          var shouldClose = (opts.autoDismiss !== false) && (actionDef.autoClose !== false);
          if (shouldClose) {
            dismiss(opts.id);
          }
        });
      })(actionBtns[i], opts.actions[i]);
    }

    // === Pause on hover (only if auto-dismiss enabled and duration > 0) ===
    if (config.pauseOnHover && opts.autoDismiss !== false && opts.duration > 0) {
      var pauseStart = 0;
  ...
  function show(opts) {
    ...
    // Apply default autoDismiss from config
    if (opts.autoDismiss === undefined) {
      opts.autoDismiss = config.autoDismiss;
    }
    
    // Apply default duration from config
