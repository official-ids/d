...
projects.forEach(project => {
  ...
  if (fs.existsSync(apiPath)) {
    // Для API нужно использовать Vercel Functions (папка /api/)
    // Создаем rewrite на специальную функцию
    
    // 1. API endpoint (должен идти ПЕРВЫМ!)
    rewrites.push({
      source: `/apps/${project}/api`,
      destination: `/api-internal/${project}`
    });
    ...
    // 3. Создаем папку api-internal если нет
    const apiInternalDir = path.join(ROOT, 'api-internal');
    if (!fs.existsSync(apiInternalDir)) {
      fs.mkdirSync(apiInternalDir, { recursive: true });
    }
    
    // 4. Копируем/создаем функцию в api-internal
    const apiContent = fs.readFileSync(apiPath, 'utf8');
    const targetPath = path.join(apiInternalDir, `${project}.js`);
    
    // Оборачиваем API в правильный формат Vercel
    const wrappedApi = `
// Auto-generated by generate-vercel-config.js
${apiContent.replace('export default function', 'export default function')}
`;
    
    fs.writeFileSync(targetPath, wrappedApi);
    console.log(`   🔌 API создан: /api-internal/${project}.js`);
  }
